...
$subject = $map['Subject'];
$customEmail = $map['CustomEmail']; // Новое поле
$messageType = $dialog->getCurrentValue(
		$map['MessageTextType']['FieldName'],
		\CBPCrmSendCustomEmailActivity::TEXT_TYPE_BBCODE
);
...
<!-- НОВОЕ ПОЛЕ: Custom Email (теперь тут) -->
<div class="bizproc-automation-popup-settings">
	<span class="bizproc-automation-popup-settings-title"><?=htmlspecialcharsbx($customEmail['Name'])?>:</span>
	<?= $dialog->renderFieldControl($customEmail)?>
	<div class="bizproc-automation-popup-settings-subtitle" style="color: #a0a0a0; font-size: 11px; margin-top: 5px;">
		Принимает строку или массив. При передаче массива будет использован первый адрес.
	</div>
</div>
<!-- Конец нового поля -->
...
<div class="bizproc-automation-popup-settings" data-role="inline-selector-html">
	...
	{
		$content = \CBPCrmSendCustomEmailActivity::decodeMessageText($content);
		$content = \Bitrix\Crm\Automation\Helper::convertExpressions($content, $dialog->getDocumentType());
	}

	if ($messageType !== \CBPCrmSendCustomEmailActivity::TEXT_TYPE_HTML)
	{
...
<input type="hidden" name="<?=htmlspecialcharsbx($map['MessageTextType']['FieldName'])?>"
	value="<?=htmlspecialcharsbx(\CBPCrmSendCustomEmailActivity::TEXT_TYPE_HTML)?>">
