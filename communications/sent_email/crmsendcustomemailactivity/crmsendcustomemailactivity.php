...
class CBPCrmSendCustomEmailActivity extends CBPActivity
{
	...
	private function closeActivity($status = 'Y', $error = '')
	{
		$this->Status = $status;
	...
	public function Execute()
	{
		...
		{
			return $this->closeActivity('N', 'Modules not loaded or text empty');
		}
		...
		if ($userId <= 0)
		{
			...
			return $this->closeActivity('N', GetMessage('CRM_SEMA_NO_RESPONSIBLE'));
		}
		...
		if (!$fromInfo)
		{
			...
			return $this->closeActivity('N', GetMessage('CRM_SEMA_NO_FROM'));
		}
		...
		$customEmailValue = $this->CustomEmail;
		
		if (!empty($customEmailValue))
		{
			if (is_array($customEmailValue))
			{
				$flatEmails = CBPHelper::MakeArrayFlat($customEmailValue);
				$to = reset($flatEmails);
			}
			else
			{
				$to = (string)$customEmailValue;
			...
			if (!empty($to))
			{
				...
				if ($ownerTypeId === \CCrmOwnerType::Deal)
				{
					...
					{
						$boundContactIds = \Bitrix\Crm\Binding\DealContactTable::getDealContactIds($ownerId);
					}
				}
				elseif ($ownerTypeId === \CCrmOwnerType::Lead)
				{
					if (class_exists('\Bitrix\Crm\Binding\LeadContactTable'))
					{
						$boundContactIds = \Bitrix\Crm\Binding\LeadContactTable::getLeadContactIds($ownerId);
					}
				}
				elseif (class_exists('\Bitrix\Crm\Service\Container')) 
				{
					...
					if ($factory && $factory->isClientEnabled())
					{
						...
						if ($item)
						{
							$boundContactIds = $item->getContactIds();
						}
					}
				}

				if (!empty($boundContactIds))
				{
					...
					if ($row = $res->Fetch())
					{
						$contactId = $row['ELEMENT_ID'];
						$comEntityTypeId = \CCrmOwnerType::Contact;
						$comEntityId = (int)$contactId;
					}
				}
			}
		}
		...
		$this->logDebug([
			'MessageFrom' => $from,
			'MessageTo' => $to,
			'TargetEntityType' => \CCrmOwnerType::ResolveName($comEntityTypeId),
			'TargetEntityId' => $comEntityId
		]);

		if (empty($to))
		{
			$this->writeError(GetMessage('CRM_SEMA_NO_ADDRESSER'), $userId);
			return $this->closeActivity('N', GetMessage('CRM_SEMA_NO_ADDRESSER'));
		}
		...
		if ($maxSize > 0 && $maxSize <= ceil($totalSize / 3) * 4) // base64 coef.
		{
			...
			return $this->closeActivity('N', 'Max size exceeded');
		}
		...
		if (!$id)
		{
			$err = CCrmActivity::GetLastErrorMessage();
			$this->writeError($err, $userId);
			return $this->closeActivity('N', 'Activity Add Error: ' . $err);
		}
		...
		if (!$sendResult)
		{
			...
			return $this->closeActivity('N', 'Mail Send failed');
		}
	...
	{
		// Порядок ключей здесь важен для автогенерируемых форм,
		// хотя кастомные диалоги его переопределяют.
		return [
			'MessageFrom' => [
			...
			'CustomEmail' => [
				'Name' => 'Email получателя (переопределить)',
				'Description' => 'Принимает строку или массив. При передаче массива будет использован первый адрес. Если заполнено, поиск в контактах игнорируется.',
	...
	public static function GetPropertiesDialogValues($documentType, $activityName, &$arWorkflowTemplate, &$arWorkflowParameters, &$arWorkflowVariables, $arCurrentValues, &$errors)
	{
		...
		$properties = [
			...
			'CustomEmail' => (string)($arCurrentValues['custom_email'] ?? ''), // Чтение значения
